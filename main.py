...
def send_from_trello_to_habitica():
    ...
    jsonList = tp.get_response_as_jsonList(
        ud.idOfList, ud.trello_apiKey, ud.trello_apiToken)
    ...
    while counter < len(validJsonElements):
        ...
        cardJson = tp.get_card_as_json(jsonElement.get("data").get(
            "card").get("id"), ud.trello_apiKey, ud.trello_apiToken)
        ...
        for idMember in idOfMembers:
            if idMember in ud.habitica_users_by_trello_member_id:
                ...
                (create_task_status_code, score_task_status_code) = hp.create_and_score_task(
                    habiticaUser["habitica_user_key"], habiticaUser["habitica_api_token"], cardJson, priority)
                print("New task sent to habitica for user " + habiticaUser["name"] + " with create_tast_status_code " + str(
                    create_task_status_code) + " and score_task_status_code " + str(score_task_status_code))
